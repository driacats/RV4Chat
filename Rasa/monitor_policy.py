...
class MonitorPolicy(Policy):
    ...
    # With respect to the super class it is initialized the socket client
    # that will connect to the monitor.
    def __init__(
        self,
        cls,
        model_storage,
    ...
    # In our case the monitor is deterministic,
    # there is no training phase, so we can simply pass it.
    def train(
        self,
    ...
    def build_user_message(self, tracker, _):
    ...
    def build_bot_message(self, tracker, _):
    ...
    # The function predict_action_probabilities is called
    # when a message arrives to rasa and returns a prediction.
    def predict_action_probabilities(
        ...
        **kwargs:Any
        ...
        if not tracker.latest_action_name == ACTION_LISTEN_NAME:
            ...
            oracle = ws.recv()
            ws.close()
        # If the monitor returns False then the message is not accepted
        # and the action used is the error one.
