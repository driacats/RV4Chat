...
import curses, threading, requests, asyncio, time, json, signal

def handle_resize(signal, frame):
    curses.endwin()  # Termina la modalità curses temporaneamente
    stdscr.refresh()  # Aggiorna lo stato del terminale
    stdscr.clear()    # Cancella lo schermo
    # Ridesegna l'interfaccia utente in base alle nuove dimensioni del terminale
    # In questo esempio, puoi semplicemente ristampare un messaggio
    stdscr.addstr(curses.LINES // 2, (curses.COLS - 20) // 2, "Terminal resized!")
    stdscr.refresh()  # Aggiorna lo schermo
    signal.signal(signal.SIGWINCH, handle_resize)  # Riregistrazione del segnale di ridimensionamento
