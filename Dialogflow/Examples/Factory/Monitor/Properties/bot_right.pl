...
match(_event, msg_user_to_bot) :- deep_subdict(_{'sender':"user",'receiver':"bot"}, _event).
...
match(_event, irrelevant) :- deep_subdict(_{'bot_action': "utter_hello"}, _event).
match(_event, irrelevant) :- deep_subdict(_{'intent': {'name': "Hello", 'confidence': _}}, _event).
