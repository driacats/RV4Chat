...
class Monitorizer:
    ...
    def generate_policy(self):
        # Make a list of the intents with WebHook flagged true (the if check in list creation)
        intent_list = [intent for intent in self.intents if self.intents[intent]]
        # If the intent is one of the original webHookUsed ones we need to forward the request
        self.policy_string = self.policy_string.replace('INTENT_LIST_PLACEHOLDER', str(intent_list))
        self.policy_string = self.policy_string.replace('MONITOR_URL_PLACEHOLDER', f'\"{self.MONITOR_URL}\"')
        # We set the webHook original URL for the forwarding
        self.policy_string = self.policy_string.replace('WEBHOOK_URL_PLACEHOLDER', f'\"{self.webHook}\"')
        if (self.level == self.LEVEL_TWO):
            bot_event_functions = ''
            ...
            header =  f'# File generated automatically on {datetime.datetime.now().strftime("%Y-%m-%d %H:%M")} by {sys.argv[0]}\n'
            header += f'#  - Agent: {self.json_agent['displayName']}\n'
            header += f'#  - Input zip: {self.input_zip.filename}\n'
            header += f'#  - Output zip: {self.output_zip.filename}\n'
            header += f'#  - WebHook URL: {self.URL}\n'
            header += f'#  - Monitor URL: {self.MONITOR_URL}\n'
            header += f'#  - Monitoring Level: {str(self.level)}\n'
            self.policy_string = f'{header}\n\n{self.policy_string}'
